package br.usp.ime.lapessc.xflow2.presentation.visualizations.scatterplot;
...
import br.usp.ime.lapessc.xflow2.presentation.visualizations.scatterplot.controls.TooltipControl;
...
//	}
	...
	private void addDataTableData() throws DatabaseException {
		...
		if(metricsSession.getAssociatedAnalysis().isTemporalConsistencyForced()){
			entries = new CommitDAO().getAllEntriesWithinEntries(metricsSession.getAssociatedAnalysis().getFirstEntry(), metricsSession.getAssociatedAnalysis().getLastEntry());
		} else {
			entries = new CommitDAO().getAllEntriesWithinRevisions(metricsSession.getAssociatedAnalysis().getProject(), metricsSession.getAssociatedAnalysis().getFirstEntry().getRevision(), metricsSession.getAssociatedAnalysis().getLastEntry().getRevision());
		}
		...
		for (int i = 0; i < entries.size(); i++) {
			...
			final Commit entry = entries.get(i);
			...
			dataTable.set(dataTable.getRowCount()-1, "Author Sequence Number", new CommitDAO().getAuthorEntrySequenceNumber(entry));
...
//		final List<Entry> entries = new EntryDAO().getAllEntriesWithinDates(this.metricsSession.getAssociatedAnalysis().getProject(), initialDate, finalDate);;
		...
		final Commit initialEntry = new CommitDAO().findById(Commit.class, 89354);
		final Commit finalEntry = new CommitDAO().findById(Commit.class, 92364);
		final List<Commit> entries = new CommitDAO().getAllEntriesWithinEntries(initialEntry, finalEntry);
//		final Map<Author, Double[]> metrics = new HashMap<Author, Double[]>();
