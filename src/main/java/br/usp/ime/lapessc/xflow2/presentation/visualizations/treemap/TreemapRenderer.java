...
import br.usp.ime.lapessc.xflow2.presentation.commons.util.TreeHierarchyBuilder;
import br.usp.ime.lapessc.xflow2.presentation.commons.util.prefuse.BorderColorAction;
import br.usp.ime.lapessc.xflow2.presentation.commons.util.prefuse.LabelLayout;
import br.usp.ime.lapessc.xflow2.presentation.commons.util.prefuse.NodeRenderer;
import br.usp.ime.lapessc.xflow2.presentation.commons.util.prefuse.TreeMapColorAction;
import br.usp.ime.lapessc.xflow2.presentation.visualizations.VisualizationRenderer;
import br.usp.ime.lapessc.xflow2.presentation.visualizations.treemap.controls.TooltipControl;
...
public class TreemapRenderer implements VisualizationRenderer<TreemapVisualization> {
	...
	public void composeVisualization(JComponent visualizationComponent) throws DatabaseException {
		...
		visualizationComponent.add(this.filesNameSearchPanel, BorderLayout.SOUTH);
	}
...
//		visualization.addDecorators("fileLabel", "tree.nodes", fileCheck, createFileLabelSchema());
	...
	private void createPredicates(final Visualization visualization, VisualTree visualtable) {
		...
		this.filesNameSearchPanel.setShowResultCount(true);
		this.filesNameSearchPanel.setMaximumSize(new Dimension(400, 20));
...
//						ColorLib.rgb(111,11,11),ColorLib.rgb(244,76,67)), 0, treeDepth);
	...
	private ActionList createLayout(Visualization visualization) {
		...
		layout.add(new LabelLayout("folderLabel", 8));
...
//	}
	...
	public void updateTree(long newSequence) throws DatabaseException{
		...
		final Commit newEntry;
		if(this.metricsSession.getAssociatedAnalysis().isTemporalConsistencyForced()){
			newEntry = new CommitDAO().findEntryFromSequence(this.metricsSession.getAssociatedAnalysis().getProject(), newSequence);
		}
		else{
			newEntry = new CommitDAO().findEntryFromRevision(this.metricsSession.getAssociatedAnalysis().getProject(), newSequence);
		}
